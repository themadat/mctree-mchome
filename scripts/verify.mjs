...
if (!index.includes('id="relationshipFixedSummary"') || !appSource.includes('$("#relationPerson1Field").hidden = editingExisting') || !appSource.includes('relationship ? initialEditField : initialSearch')) fail("Existing relationship editing is no longer type-aware");
...
if (!appSource.includes('data-symbol="lineageCheck"') || !css.includes(".relationship-actions .profile-action-icon .sf-symbol { width: 2rem;")) fail("The larger relationship icons or Check & Update icon regressed");
