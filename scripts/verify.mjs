...
if (!iconsSource.includes("outlineRoot: __OUTLINE_ROOT") || !iconsSource.includes("M9.77051 24.947L33.7012 24.947") || !appSource.includes('"outlineRoot", "Selected as root"')) fail("The requested Outline root symbol is missing");
if (!iconsSource.includes("outlineExpand: __OUTLINE_EXPAND") || !iconsSource.includes("outlineCondense: __OUTLINE_CONDENSE") || !iconsSource.includes("M1.68945 20.2344L12.2656 14.043") || !css.includes('[data-symbol="outlineExpand"] .sf-symbol') || !css.includes("transform: rotate(90deg)")) fail("The requested rotated Outline expand and condense symbols are missing");
...
if (!appSource.includes('class="button small outline-action"') || !appSource.includes('(collapsed ? "chevronRight" : "chevronDown")') || !iconsSource.includes("chevronRight: __CHEVRON_RIGHT") || !iconsSource.includes("chevronDown: __CHEVRON_DOWN")) fail("Outline controls must use the shared icon catalog and chevron branch states");
if (!css.includes(".outline-row") || !css.includes(".outline-scan-bar") || !css.includes(".outline-action-icon") || !css.includes(".print-outline .outline-row") || !css.includes(".workspace-view-switch .tree-option-action { display: inline-flex; flex-direction: column;")) fail("Outline screen, controls, or stacked view styling is missing");
