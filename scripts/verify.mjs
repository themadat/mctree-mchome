...
if (config.controls.maxPrintOutlineRows !== 40 || config.controls.maxPrintDirectoryUnits !== 70 || config.controls.maxPrintGroupUnits !== 26 || config.controls.maxPrintGroupPeoplePerSection !== 42) fail("Paged report preview limits are invalid");
...
if (!appSource.includes("const generationNumber = relationshipGeneration(person)") || !appSource.includes("G' + generationNumber") || !appSource.includes("function outlineLifeDatesHtml") || !appSource.includes('class="outline-life-dates is-stacked"') || !css.includes(".outline-life-dates.is-stacked { display: grid;")) fail("Outline generation labels or compact stacked life dates regressed");
...
if (!appSource.includes('target.closest("[data-outline-condense-lineage]")') || !appSource.includes("if (!lineageKeys.has(key) || key === selectedKey)") || !appSource.includes("condenseLineage.disabled = !result.availableHighlightPath")) fail("Condense Lineage behavior or availability regressed");
...
if (!appSource.includes('class="print-directory print-directory-page print-sheet-page"') || !appSource.includes('class="print-atlas print-atlas-page print-sheet-page"') || !appSource.includes('class="print-outline print-outline-page print-sheet-page"') || !appSource.includes("function printReportMetaHtml") || !appSource.includes("return 5 + Math.max(0, household.partners.length - 1);") || !css.includes(".print-preview-document > .print-directory-page") || !css.includes(".print-preview-document > .print-outline-page { box-sizing: border-box; width: min(8.5in, 100%); height: 11in;") || !css.includes("body.printing-outline .print-outline-page { box-sizing: border-box; width: 7.5in; break-after: page;") || css.includes("body.printing-outline .print-outline-page { box-sizing: border-box; width: 7.5in; height:") || css.includes("body.printing-directory .print-directory-page, body.printing-groups .print-atlas-page { box-sizing: border-box; width: 7.5in; height:") || !css.includes("}\n\n.print-atlas > h2")) fail("Directory, Groups, or Outline no longer expose accurate portrait preview page breaks without native blank sheets");
