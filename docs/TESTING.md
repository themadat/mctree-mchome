...
| Tree and Outline | Outline is absent from the application toolbar and sits beside Tree in the central view switch; its controls have icons; every person occupies one compact line; only the latest spouse or partner appears; no partner placeholder appears; branch collapse and lineage highlighting remain correct |
| Print | Preview Directory, Outline, Groups, and Tree; half-inch margins; repeated directory header; rows/cards never split; complete expanded compact Outline order; maps absent from Directory; current tree mode/settings/zoom reflected; no controls/P ids/source/Notes; all allowed people and contacts present |
