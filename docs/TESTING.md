...
- [ ] Owner and every named Editor enable Audit, family-record editing, family Notes, PDF, and audited family publication. The modal begins with same-line Signed in as/permission, encrypted family file, and GitHub connection summaries; the latter two are green or red according to status, and activating GitHub expands its connection details directly below the row. Owner permission reads Admin. Owner alone sees Owner Recovery and can create, rename, rotate, and revoke up to 20 grants in each Editor, Member, and Viewer role.
...
- [ ] Opening an unchanged hosted family shows `No Updates` and disables the symbol-labelled Update button. Family title/root, Notes, person, relationship, place, and residence changes appear automatically in Detailed Changes; Favorites and other personal device preferences do not enable Update.
- [ ] A valid family publication requires Summary of What Changed, records Owner publication as `Admin` or an Editor publication under that signed-in username, appends the generated details to the audit description, advances exactly one dataset patch, appends one `published-hosted-family` event, validates and encrypts both packages, writes one ciphertext vault, and keeps existing grants usable.
...
- [ ] Add Person fits a 1280 × 720 desktop viewport without body scrolling, including one physical address row: Birth, Current, and Preferred remain single-line and weighted `5/30/30/30/5`, Maiden aligns to the Last column, Birth input mirrors each untouched Current/Preferred field, and editing one target stops only that target from mirroring.
...
- [ ] Generic Add Person can select any existing Parents, Spouses / Partners, and Children; duplicate, self, missing-reference, second-Lineal-parent, and ancestry-cycle failures leave both person and relationships unmodified.
