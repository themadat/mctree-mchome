...
- [ ] Member and Viewer omit the Audit button, `E` shortcut, and modal; omit person and family Notes entirely—including profile section, header, `N` shortcut, search, Help topic, and populated dialog—disable record mutations; and omit routine recovery ZIP import/export, PDF, Developer data, publishing, GitHub-token, and passphrase-management surfaces. Imported Source does not participate in their search. Search, favorites, and display preferences remain available.
...
- [ ] Owner and every named Editor enable Audit, family-record editing, family Notes, recovery ZIP, PDF, and audited family publication. The modal begins with same-line Signed in as/permission, encrypted family file, and GitHub connection summaries; the latter two are green or red according to status. Owner alone can create, rename, rotate, and revoke up to 20 grants in each Editor, Member, and Viewer role.
- [ ] In Owner Developer Mode, the role pill opens a keyboard-navigable Owner/Editor/Member/Viewer menu. Editor preview hides Owner-only access management; Member and Viewer previews apply their normal read-only, PII, Notes, PDF, Audit, source, and Developer gates. Preview never changes the vault, package access, credentials, or audit actor and resets when Developer Mode turns off or the page reloads.
...
- [ ] Selected-person Add, Connect, Edit, and Delete controls are enabled only when `features.familyEditing` is true and active hosted access is Owner or Editor; they are disabled and greyed in both read-only modes. Set as home is absent.
- [ ] Add Person fits at ordinary desktop height without body scrolling: three name rows remain single-line and weighted `5/30/30/30/5`, Birth input mirrors each untouched Current/Preferred field, editing one target stops only that target from mirroring, and the form ends with Editor Notes → Details.
- [ ] Add/Edit Person omits Gender, Pronouns, birth/death place, manual qualifiers, and lineage background while preserving those values on an existing person. Birth and death descriptors derive as year/month/day/partial/UNKNOWN/NONE, a death value selects deceased, valid `?` partials save and round-trip, and malformed or impossible dates are marked red and cannot save.
