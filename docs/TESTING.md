...
| Print | Every Directory, Labels, Groups, Outline, and Tree action opens an in-app preview before native print; the preview header places icon-and-text Print beside Close; Tree and Outline actions are labelled Print and share the printer symbol while the Tree view retains its tree symbol; half-inch margins; repeated directory header; rows/cards never split; complete expanded compact Outline order with full names wrapping instead of clipping; maps absent from Directory; Tree pages never exceed eight generation levels or ten people in one generation row, repeat a boundary level vertically, keep partner groups together when possible, keep a descendant branch with its parent when dividing horizontally, collision-pack repeated parent/spouse context without overlapping cards, compact Details metadata onto one row except for Developer references, label level/across sections, and create more pages at higher zoom; no controls/P ids/source/Notes; all allowed people and contacts present |
