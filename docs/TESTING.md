...
- [ ] Summary cards are the default; both Summary and Details stay narrow, balance names with four or more parts across three fitted lines without widening cards, grow generation rows as needed, and use larger lifespan text. Living people show only their birth year; other unknown years use `????`. Lineal cards retain the standard living or deceased fill and use a bold muted-red outline plus a compact lineage symbol on the lifespan row.
