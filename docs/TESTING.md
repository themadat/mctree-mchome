...
| List/search | Global, List, and roadmap searches stay usable during the session but start empty after reload; every access-mode ZIP omits search text, including when the publisher has an active search; importing older packages/local snapshots/recovery with searches starts empty; saving/exporting preserves live searches and device favorites; Fuzzy three-name search; favorites picker; living/contact filters; first/last sort; alphabet jump; role-redacted search fields |
...
| Print | Every Directory, Labels, Groups, Outline, and Tree action opens an in-app preview before native print; Labels preserve Avery 5260 positions on full and partial sheets (1, 30, 31, 60, 61, 90, and 91 households), with native Chromium and WebKit PDFs matching the expected sheet counts and no blank pages; Directory preview and native page counts agree for short cards, wrapped contacts/addresses, and multi-person households, filling pages by measured card heights; its compact title and header spacing fit at least one additional ordinary address card compared with 0.0.1.141; Groups fills each page by measured six-person rows, with no fixed 42-person chunks, no repeated generation heading within the same root/branch on a page, and continued labels only across page boundaries; verify mixed roots/branches, long names, final partial rows, complete cards, and matching preview/native counts; the preview header places icon-and-text Print beside Close; Tree and Outline actions are labelled Print and share the printer symbol while the Tree view retains its tree symbol; Directory, Groups, Outline, and Labels use Letter portrait while Tree alone uses Letter landscape; Labels preserve the Avery 5260 three-column, ten-row geometry and print exactly one physical page per planned sheet (check 1, 30, 31, 60, and 181 synthetic households, including a partial final sheet, in Chromium and Safari/iOS); no alternating or trailing blank pages; Directory, Groups, and Outline show fixed Letter preview sheets, page counts, and preview gaps, then use matching native breaks without alternating blank pages; they reuse native print typography/grids so simulated fill is accurate; Outline previews full Letter sheets but prints content-sized blocks inside half-inch page margins; repeated report headings; rows/cards never split or overflow their sheet; the native PDF page count equals the planned Outline page count with no orphan continuation sheets; check the initial macOS Safari print-dialog thumbnails as well as saved PDFs, with browser headers/footers both enabled and disabled at 100% Letter scaling (saved PDFs alone can miss Safari's initial margin mismatch); the temporary `@page` rule must be available before print media activates and be removed after printing; Outline fills portrait pages with up to 44 currently expanded compact rows, omits the chevron gutter, keeps Generation close to the person card, and wraps full names instead of clipping; maps absent from Directory; Full Tree pages never exceed eight generation levels or ten people in one generation row, while focused Lineage pages never exceed twelve levels or sixteen people across; Tree always prints at 100% regardless of live zoom, repeats a boundary level vertically, keeps partner groups together when possible, keeps a descendant branch with its parent when dividing horizontally, collision-packs repeated parent/spouse context without overlapping cards, compacts Details metadata onto one row except for Developer references, and labels level/across sections; no controls/P ids/source/Notes; all allowed people and contacts present |
