...
- [ ] Summary cards are the default; both Summary and Details stay narrow, balance names with four or more parts across three fitted lines without widening cards, grow generation rows as needed, and show birth/death years with `????` for either unknown year. Lineal cards retain the standard living or deceased fill and use a bold muted-red outline plus a compact lineage symbol on the lifespan row.
