...
| Tree and Outline | Outline is absent from the application toolbar and sits beside Tree in the central icon-over-label view switch; Root/Expand/Condense use their requested shared icons; branch controls show `>` when condensed and `v` when expanded; every person occupies one compact line; only the latest spouse or partner appears; no partner placeholder appears; branch collapse and lineage highlighting remain correct |
