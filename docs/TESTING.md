...
- [ ] Hosted unlock, Update, access publication, and Bulk Upload clear any `mcfamily.recovery.v5` snapshot before saving the one browser working copy; hosted Developer Mode hides manual recovery-snapshot controls, while local-only setup retains them.
