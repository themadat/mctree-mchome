...
- [ ] Private Viewer and Redacted Viewer omit person and family Notes entirely—including profile section, header, `N` shortcut, search, Help topic, and populated dialog—disable record mutations, and omit routine recovery ZIP import/export, PDF, Developer data, publishing, GitHub-token, and passphrase-management surfaces. Imported Source does not participate in their search. Search, favorites, display preferences, Lock, and audit history remain available.
...
- [ ] Owner and every named Editor enable family-record editing, family Notes, recovery ZIP, PDF, and audited family publication. Owner alone can create, rename, rotate, and revoke up to 20 grants in each Editor and viewer role.
...
- [ ] Lock clears decrypted browser state and recovery, reloads, and requires a fresh online passphrase check without publishing or changing the hosted vault. It preserves only device-local dismissed hint ids, dismissed What’s New versions, and Directory visibility; no family values enter that preference record.
- [ ] Dismissing a contextual hint or a specific What’s New banner and opening or closing Directory survive hosted replacement, Lock, and reload on the same browser profile. A newer release still shows its own banner, while Reset Preferences and Erase Everything clear the saved choices.
