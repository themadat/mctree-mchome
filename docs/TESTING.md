...
| Print | Preview Directory, Outline, Groups, and Tree; half-inch margins; repeated directory header; rows/cards never split; complete expanded compact Outline order with full names wrapping instead of clipping; maps absent from Directory; Tree pages never exceed six generation levels or eight people in one generation row, repeat a boundary level vertically, keep partner groups together when possible, label level/across sections, and create more pages at higher zoom; no controls/P ids/source/Notes; all allowed people and contacts present |
