...
- [ ] Bulk publication requires an audit summary, appends one `published-bulk-package` event recorded by Admin, saves the previous family as recovery, keeps every active grant usable, generates independently redacted Viewer data, checks remote revision and SHA, and replaces local state only after the encrypted GitHub write succeeds.
