...
5. `core/storage.js` loads and autosaves one browser working copy and retains recovery snapshots only for local-only setup.
...
Startup checks only `mcfamily.state.v13` and the matching local-only `mcfamily.recovery.v5` snapshot. State v13 passes through normalization, sanitization, and validation; every other state version is rejected. McFamily removes its own older versioned state/recovery keys before loading so obsolete copies cannot consume the local-storage quota; hosted unlock also removes the current recovery key before saving the decrypted working copy. A corrupt local-only current copy falls back to its recovery snapshot or to the uninitialized gate. Ordinary mutations update metadata and are saved locally with a short debounce.

Hosted person and relationship deletion change only the browser working copy until Update, so GitHub remains the recovery source and no full local snapshot is created. Local-only deletion can still write one last-known recovery snapshot. Favorite person IDs ordinarily live in normalized UI state; Developer Mode can additionally download a small private `mcfamily-favorites` JSON envelope and restore that exact ID set without depending on browser storage.
...
- Family Tree: a two-axis-scrollable SVG focus view with grouped numeric ancestor and descendant depths defaulting to 10, a right-aligned editable zoom and Out/In/Fit group, an optional display-only Non-Lineal Lines overlay, bold muted-red Lineal outlines with corner symbols, compact address/phone/email availability symbols, or an overview containing all connected components and isolated people.
