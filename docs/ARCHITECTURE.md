# Architecture

McFamily is one static application shell with no runtime dependencies. Scripts attach modules to `window.LocalApp` in the order listed by `index.html`; `app.js` initializes them after `DOMContentLoaded`. All data-bearing DOM output uses text nodes or sanitized HTML.

## Runtime layers

```text
config + icons
      ↓
utils → state → storage
      ↓       ↘
family  portability → cloud
      ↘ components ↙
          app
           ↓
          pwa
```

`config.js` is the single source for identity, versions, limits, storage keys, access and relationship vocabularies, help, release, and roadmap content. `state.js` owns the in-memory shape. `portability.js` is the trust boundary for packages. `cloud.js` is the trust boundary for encrypted hosted access.

## Application state v14

The normalized state contains:

```text
schemaVersion
meta
  appVersion, buildId, timestamps, lastMutationId
  package: format, version, datasetVersion, accessMode, auditHistory
workspace
  family: title, initializedAt, homePersonId
  people, relationships, places, residences
  documents: one Notes document
preferences
  appearance, controls, installation
ui
  selection, tree/List settings, panel sizes, favorites, support state
modules
  family, documents, roadmap
```

State v14 deliberately omits retired generic records and sync tombstones. `normalize()` accepts only the exact current schema, rebuilds derived person addresses from places/residences, consolidates Notes, sanitizes values, and validates relationship/place references and ancestry cycles.

## Persistence modes

Local development (`?local=1` on localhost) stores full state in `mcfamily.state.v14` with recovery in `mcfamily.recovery.v6`. Older versioned McFamily state/recovery keys are removed rather than migrated. Current favorites and List visibility also live in the PII-free device-preference record.

Hosted mode keeps decrypted family state and the publication baseline in memory. Lock/reload clears it. Only compact device preferences, non-secret connection coordinates, a session-scoped token, and a “hosted seen” flag may persist. Ordinary mutation is local until Update successfully publishes; revision/SHA verification prevents overwriting another publication.

## Data model

People store five-part Birth/Lineal, Current/Legal, and Preferred/Display names plus maiden last name, life data, contacts, heritage, notes, and imported source. Places store normalized postal fields, an optional household phone, notes, and source. Residences connect people to places. Relationship records are either parent-child or partner and carry their own vocabulary, dates, order, and source metadata.

Family derivation uses relationships at render time. Lineage ids are validated paths from one Lineal parent; Non-Lineal parents do not extend the path. Multiple partners and multiple Non-Lineal parents are supported. State normalization applies limited current-data inferences such as presumed deceased status without rewriting source values.

The exact five-file ZIP/CSV contract is in `MCFAMILY_CSV.md`.

## Encrypted hosted access

The public data repository stores one `mcfamily-encrypted-vault` JSON object containing metadata, passphrase grants, and AES-GCM ciphertext for full and redacted packages. The website fetches it anonymously with `cache: no-store`.

Each grant uses PBKDF2 to wrap only the keys permitted by its role. On sign-in, the browser tries the entered passphrase against all grants and requires exactly one match; labels and roles are not exposed before authentication. Full and redacted package bytes are independently encrypted. Every decrypted package still passes the exact package validator before becoming state.

Non-Admin grants always open with Developer Mode disabled even if the encrypted full-data package was saved while an Admin had it enabled. Editors may enable it deliberately for their current session; read-only roles cannot use it.

Settings exposes Data Cleanup as its own sidebar section to Owner and Editor roles without requiring Developer Mode. The read-only diagnostics classify suspicious-but-loadable relationships, mismatched current postal addresses for living ongoing partners, unknown or partial life dates, missing names or exact placeholder name parts, and invalid Lineage IDs. Every placeholder occurrence identifies its affected Birth, Current, Preferred, or Maiden field even when the person also has a valid name. Every category is independently collapsible and links back to the affected editor; Member and Viewer roles cannot access the section.

Admin publishes access changes and bulk packages; Admin/Editor publish ordinary changes. Publication verifies the configured repository/token and current remote revision, appends an audit event, derives the redacted package, encrypts both views, and writes through the GitHub Contents API. Git history provides rollback of publications. The app cannot audit anonymous reads or protect a shared passphrase from being forwarded.

## UI and print

The tree is vanilla SVG with relationship-derived layout, a 40px vertical generation gap, pan/zoom, focus/full modes, keyboard navigation, five-percent percentage steppers, an icon-over-label Reset action that restores a natural 100% view between In and Fit, and accessible node descriptions. Its horizontal placement groups partners into person components and siblings into parent-specific branches, centers each branch on the midpoint of its recorded parent nodes, orders branches by those parent midpoints so a wide later family cannot cross ahead of an earlier parent, then applies ordered interval packing to remove collisions between neighboring family branches. A central icon-over-label Tree/Outline control switches both the content and its matching toolbar to the DOM-based, depth-first descendant view over the same relationship index; Outline has no separate title header, reuses Tree Name Preferences, constrains its compact toolbar-aligned searchable root picker to the Root Ancestor's Lineal descendant set, shows the selected root's resulting people count in a pill, and keeps only the latest spouse or partner beside each compact row. Condense Lineage leaves the selected person's entire descendant subtree expanded while collapsing branches outside their direct ancestor path. Spouse cards match Lineal card widths and have a neutral edge, relationship type/date labels occupy two centered lines across the partner bar, indentation replaces parent-child connectors, and right/down chevrons expose branch state. A new person launched from a profile relationship choice defaults to the related person's current address through an explicit checkbox; edits detach that copy from the source place before save. The List and profile are DOM views over the same state; no separate search index is persisted. Panels are responsive, collapsible, and resizable.

Print output is generated into a print-only DOM, cloned into an in-app preview for every print action, and sent to `window.print()` only from the preview header’s Print button. Directory contains household contact cards, Outline emits the current expanded descendant order for its chosen root with up to 44 compact rows per portrait sheet and permits long names to wrap without ellipsis, and Groups contains the generation maps. These three reports and Labels use portrait Letter pages; Tree alone uses landscape. Directory, Outline, and Groups render fixed portrait preview sheets with visible gaps, repeated report headings, page counts, matching native page breaks, and the same compact typography and grids in preview and print, so page fill is representative. Outline owns an explicit 8.5-by-11-inch native sheet with half-inch internal padding on a zero-margin `@page`; its hidden paginator reserves a two-pixel measurement guard inside that same page boundary so print engines cannot move a fitted final row onto a continuation sheet. Directory and Groups retain content-sized native sections with half-inch `@page` margins. Tree plans semantic landscape pages from the live SVG layout at a fixed print scale of 100%, independent of interactive zoom: Full Tree permits at most eight generation levels and ten people across, while focused Lineage permits twelve levels and sixteen people across. A boundary generation repeats between vertical sections, partner groups remain together when possible, descendant clusters follow their parent into the same horizontal section, and available parent/partner context repeats above the divided branch. Print-only context nodes are cloned, grouped with their partners, collision-packed by generation, and re-centered before page bounds and edges are calculated, so crowded repeated context cannot overlap and the live Tree layout is never mutated. Detailed Tree cards use one compact metadata row for contact symbols, lifespan, and the Lineal symbol; only Developer Mode reserves a separate reference row. Explicit portrait and landscape `@page` rules avoid unsupported margin-box declarations. CSS uses half-inch page margins, repeated directory headings, `break-inside: avoid` rows/cards, and print-safe colors.

The Save dialog keeps session identity, hosted dataset/date, and GitHub connection state in two-line tiles within its persistent header. The main publication section is a compact ordered workflow—one-line publisher, equal-height next patch, Bulk Upload, Update—followed by an inline summary input and one full-width row per calculated change.

## PWA and deployment

`sw.js` precaches only the static shell. Navigations use network-first with offline shell fallback; same-origin static assets use network-first with cache fallback. Vault/GitHub requests are cross-origin and are never intercepted.

The repository's GitHub Pages setting uses **GitHub Actions** as its only source. `.github/workflows/deploy-pages.yml` publishes the repository root after each push to `main`; **Deploy from a branch** must remain disabled so there is no duplicate deployment or notification. Its workflow name includes McFamily and the current app version for GitHub Mobile notifications, and its run title mirrors the versioned commit subject in Actions. Because every committed path is public and may be web-addressable, private family data and credentials must never enter this repository.

## Security boundary

Static encryption reduces accidental exposure but is not an identity system. Security depends on private passphrases, a scoped GitHub token on editing devices, trusted client code, and prompt revocation when a secret is shared. PII must never enter this repository, issue trackers, logs, screenshots, or test fixtures.

### Personal Tree view

Shared five-file packages omit `ui`; imports ignore older embedded interface state. `mcfamily.user-view.v1.<grant-id>` stores only an allowlist of person IDs and Tree display settings on the current browser. Hosted unlock chooses the actual access grant before restoring these settings. New grants start with neutral defaults; invalid/deleted selections are cleared. Saving family data preserves the current user’s saved view, and Reset Preferences affects only that user’s Tree record. No names, search text, family records, credentials, or keys belong in the view record. Existing device favorites and List visibility keep their separate storage. These preferences do not synchronize across devices.
