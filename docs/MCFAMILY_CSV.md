...
Admin may instead choose Bulk Upload in Audit for a complete newer dataset package. McFamily runs the same strict package validation before showing a record-count and change review, requires an audit summary, verifies the GitHub revision and SHA, saves recovery, encrypts full and redacted views with the existing active grants, and only then replaces hosted and local data. The dataset 17 cutover build can open the existing dataset `16.0.x` hosted vault for Admin only; ordinary imports remain dataset 17-only, and normal Update plus access changes remain blocked until Admin publishes the validated v17 package.
