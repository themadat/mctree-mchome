...
Normalized dates accept `YYYY`, `YYYY-MM`, or `YYYY-MM-DD`. Other source date values are retained in source details and reported rather than guessed. Notes and data-quality notes remain distinct. Every original source column is also retained in the person's source field map so an import does not silently discard information.
