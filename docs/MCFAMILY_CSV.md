...
Current McLineage rows use stable `P` record references such as `P001`. `lineage_id` is the complete lineage path from the oldest recorded ancestor to that person, with every child position stored as a two-digit dotted segment. For example, P044 is `01`, P228 is `01.01`, and P501 is `01.01.01.03.02.02.04.02.01.01`. `lineage_parent_id` directly references the parent row's `record_id`; `lineage_parent_name_full` repeats that person's full name for human readability but is not authoritative. Former legacy page references are prefixed to `data_quality_notes` and are no longer a separate column.
