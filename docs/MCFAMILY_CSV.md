...
`place-id` is a unique `L` reference with at least four digits. At least one physical-address field must be present. People do not embed these address fields; `McResidences.csv` links them. A Place may also own one shared landline, stored in McMetadata `settings-json` under `placeDetails` so the five dataset 17 schemas remain unchanged. Every `placeDetails` key must resolve to McPlaces and contain exactly one non-empty string `phone`; orphan, empty, or unexpected fields reject the package. Dataset 17 uses McPlaces schema `2.0.0`; its final three source fields preserve the McDirectory row lookup number, pcard, and source Notes. Source modification date and actor remain separate. Those source properties are visible only to Owner/Editor while Developer Mode is enabled.
