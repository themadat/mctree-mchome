...
Current version: `0.0.1.29` (`major.minor.patch.build`).
...
- Supports partial/fuzzy matches that return the tree to Focus, collapsible side panels, resizable desktop directory/tree/profile splits, and compact tree cards that stack each name word on its own line.
...
- Imports known and question-mark partial source dates; known deaths, explicit `UNKNOWN` death dates, G0-G4 lineage people, and people older than 100 are deceased or presumed deceased, with profile and print age estimates when birth data permits.
