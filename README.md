...
Current version: `0.0.1.46` (`major.minor.patch.build`).
...
- Can hide `99`-lineage people from Full Tree with a persisted `?? Lineal` control whose outlined symbol remains fixed; the control is hidden in Lineage view, while those people remain available in directory and search. Printable atlases omit unresolved `99`/`??` lineage branches.
...
- Shows partial source dates such as `December ??, 1979`, keeps the compact Age property on one line, and fills unknown identity properties with `UNKNOWN`. Living people show only their birth year in directory and tree lifespans rather than an unknown death year.
...
- Exports a complete editable McFamily CSV and creates a print-only family atlas with brown deceased entries, faded-red Lineal outlines, and Newton/Albon/Lucian orientation highlights. Jon Couts remains in the directory but is omitted as a map root.
