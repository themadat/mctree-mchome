...
- Opens only after importing one validated McFamily ZIP containing the five required CSV files and at least one person on first launch.
...
On a fresh browser profile, McFamily intentionally has no demo family or blank-workspace bypass. Select the current private package described in [`docs/MCFAMILY_CSV.md`](docs/MCFAMILY_CSV.md).
...
The primary maintainer owns one canonical private ZIP:
