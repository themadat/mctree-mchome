...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.17` uses schema v7, strict private-CSV onboarding, stable P-formatted McLineage records with direct lineage-parent references, absolute root-based generations, fuzzy search with persistent favorite people, a header-opened sortable directory with A–Z jumps, separate ancestor/descendant depths, optional display-only co-parent branches, a two-axis-scrollable lineage-ordered condensed/detailed Family Tree, a resizable desktop tree/profile split, aligned lineage profile rows, compact profile family groups, a complete native CSV transfer format, a temporarily read-only family directory, and compact top-sibling-labelled print maps. There is no backend, account, GitHub Sync, cloud database, or runtime dependency.
...
- The header Directory control sits left of Search and opens the directory; Favorites sits right of Search. Both use an icon above a visible label. The directory module bar contains only the result count and close control. Desktop Directory and Profile panes collapse independently. Directory sorting supports first or last name, with an A–Z rail based on the filtered results.
...
- Tree nodes default to condensed given/family/lifespan cards and can switch to detailed cards; neither mode uses portrait or initial placeholders. Generation rows sort by numeric source lineage ID before fallback placement. Recorded partners sit together when possible, with past partners chronologically on the left and exactly one current spouse on the right; married links are solid and divorced links are dotted. Cleaned McLineage imports treat all populated spouse slots before the last as prior/divorced and apply the source status to the last. Focus view has independent Ancestors and Descendants depth controls, and person search selections return to Focus.
...
- Lineage shows the two-digit source ID in person-to-root order with the selected person's first segment bold. Current cleaned sources use direct `lineage_parent_id` P references and retain `lineage_parent_name_full` only for readability; legacy name/prefix sources remain importable. A single Names heading introduces paired name/reading rows whose cells share height and center vertically; ancestor, sibling, and descendant totals span beneath both columns on one line. Every resolved name remains linked. George McMillen (1745) is Gen 0, James is Gen 1, George (1818) is Gen 2, and Albon/Newton/Lucian are Gen 3. Unknown positions omit the ordinal and read `Child of …`.
- Profile relationships use compact, initially open Parents, Siblings, Partners, and Children groups with names only, in that order. The Parents display may infer a recorded parent's partner as a likely co-parent when partnership dates overlap the child's birth; this is a display-only inference and does not create a relationship record. Person Notes appear after Relationships at the bottom of both screen and print profiles.
