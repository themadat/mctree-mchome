...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.15` uses schema v7, strict private-CSV onboarding, cleaned McLineage mapping, absolute root-based generations, fuzzy search with persistent favorite people, a header-opened sortable directory with A–Z jumps, separate ancestor/descendant depths, a two-axis-scrollable lineage-ordered condensed/detailed Family Tree, aligned lineage profile rows, compact profile family groups, a complete native CSV transfer format, a temporarily read-only family directory, and compact top-sibling-labelled print maps. There is no backend, account, GitHub Sync, cloud database, or runtime dependency.
...
- Tree nodes default to condensed given/family/lifespan cards and can switch to detailed cards; neither mode uses portrait or initial placeholders. Generation rows sort by numeric source lineage ID before fallback placement. Recorded partners sit together when possible, with past partners chronologically on the left and the current spouse on the right; married links are solid and divorced links are dotted. Focus view has independent Ancestors and Descendants depth controls.
...
- Profile relationships use compact, initially open Parents, Partners, Siblings, and Children groups with names only, in that order. The Parents display may infer a recorded parent's partner as a likely co-parent when partnership dates overlap the child's birth; this is a display-only inference and does not create a relationship record. Person Notes appear after Relationships at the bottom of both screen and print profiles.
