...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.16` uses schema v7, strict private-CSV onboarding, cleaned McLineage mapping, absolute root-based generations, fuzzy search with persistent favorite people, a header-opened sortable directory with A–Z jumps, separate ancestor/descendant depths, optional display-only co-parent branches, a two-axis-scrollable lineage-ordered condensed/detailed Family Tree, a resizable desktop tree/profile split, aligned lineage profile rows, compact profile family groups, a complete native CSV transfer format, a temporarily read-only family directory, and compact top-sibling-labelled print maps. There is no backend, account, GitHub Sync, cloud database, or runtime dependency.
...
- Lineage shows the two-digit source ID in source order with the selected person's final segment bold. A single Names heading introduces paired name/reading rows whose cells share height and center vertically; ancestor, sibling, and descendant totals span beneath both columns on one line. Every resolved name remains linked. George McMillen (1745) is Gen 0, James is Gen 1, George (1818) is Gen 2, and Albon/Newton/Lucian are Gen 3. Unknown positions omit the ordinal and read `Child of …`.
