...
This repository is McFamily, a static family atlas. Version `0.0.1.62` uses a latest-only schema v13 browser namespace and strict dataset 16.0.x packages internally. Normal recipients use one public Pages link: `core/cloud.js` anonymously fetches a ciphertext-only JSON vault from the public `themadat/app-data` repository, then PBKDF2 plus AES-GCM opens the selected Owner, Editor, Private Viewer, or Redacted Viewer grant entirely in the browser. Owner and Editor decrypt the full record and may publish; Owner alone adds, rotates, or removes grants. Private Viewer decrypts full PII read-only. Redacted Viewer receives only a separately encrypted package where places, residences, contacts, family Notes, and unstructured record notes were removed before encryption. Read-only modes omit routine recovery ZIP, import, PDF, developer-data, and publishing controls. The passphrase gate reappears on every reload and the encrypted vault is never service-worker cached. There is no custom backend, account provider, cloud database, or runtime dependency.
...
- Owner and Editor grants wrap both full and redacted data keys so an existing publisher can update both ciphertext payloads without knowing viewer passphrases. Private Viewer wraps only the full key; Redacted Viewer wraps only the redacted key. Removing a grant blocks future reloads but cannot erase previously viewed or copied information. Viewer sign-ins are not centrally auditable without a backend or viewer write credential.
...
- Access & Audit always shows current access, encrypted vault status, Lock, and recent in-package audit. Owner additionally sees passphrase generation/rotation/revocation. Owner and Editor see current-family publication, GitHub connection, and recovery ZIP controls. Viewer modes expose none of those routine import/export/publish surfaces. Publications require an actor/summary, advance one `16.0.x` patch, append `published-hosted-family`, `created-hosted-access`, or `updated-hosted-access`, revalidate both packages, and use vault revision plus GitHub SHA to reject stale writes. Tokens/settings remain outside state, vault, recovery, and the service worker.
...
- `features.familyEditing` is true, but mutations additionally require Owner or Editor hosted access (both normalize to state access mode `editor`). Keep Add, Connect, Edit, Delete, family-title, and Notes changes guarded in code and visibly disabled in Private Viewer and Redacted Viewer. Hide routine recovery ZIP, import, PDF, Developer-data, and publication controls in both viewer modes. Personal preferences and favorites remain local. Do not expose Set as home in the selected-person profile.
