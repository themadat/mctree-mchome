...
- Editors can add/edit people, places, contacts, and relationships. A profile’s left-aligned relationship toolbar uses Add, Edit, and Delete. Add groups Parent/Child/Partner under New Person and Existing Person; creating a New Person from one of those relationship choices shows a default-on Autofill from related person checkbox immediately above Addresses and copies the related person's current address into the form, while opting out removes that untouched copy and editing it detaches the new address from the related person's shared place. After an Existing Person role is selected, the current profile person and relationship type stay fixed while only the other-person search and relevant relationship fields remain editable. Edit and Delete group the person’s current links under Parents, Partners, and Children before continuing to the focused form or delete confirmation. Connect Existing People has fuzzy name/date/lineage/id search. A nameless Unknown person may preserve a known partnership without inventing spouse details. Parent-child links use Biological, Adopted, Step, Foster, Guardian, or Unknown status. Only Biological and Adopted links may drive Lineage ID allocation or Lineal tree styling; legacy Lineal Step/Foster/Guardian/Unknown values remain loadable but are treated as Non-Lineal until an editor corrects them. Children remain in birth order, but Step/Foster/Guardian/Unknown children show their relationship type and do not consume `01`, `02`, etc. Sibling lists include Self, place unknown birth years last, and use the parent carrying the Lineal sibling branch as their relationship context, so Step children remain Step while Biological/Adopted siblings receive birth-order numbers. Step-only children do not display a stale Lineage ID, and Check & Update removes it. Marriages without a recorded ending remain current even when partner layout places another relationship first. Parent changes rebuild affected birth-ordered sibling branches, and Editor/Admin can force a full branch check from a profile. Settings gives every editing role a dedicated Data Cleanup sidebar with independently collapsible checks for unknown/invalid relationships, mismatched current partner addresses, unknown/incomplete birth and death dates, unknown names, and bad Lineage IDs. Preferred follows changed legal name; maiden last initially follows birth last. Each saved field-level change produces an audit line.
...
- Directory prints only the compact Directory of McMillen Clan contact cards. Outline is an interactive, depth-first descendant list with compact single-line rows that follow the shared Name Preferences, stable Root-Ancestor-relative generation numbers, life dates, the latest spouse or partner, centered two-line relationship type/date labels, scan bars, chevron branch controls, and optional selected-lineage highlighting; fully known birth and death dates stack with readable line spacing inside the unchanged row height. A newly selected visible person is revealed and vertically centered, including a spouse shown beside a Lineal person. Spouse cards match Lineal card widths, do not carry the Lineal accent, and people without partners have no placeholder copy. Its compact toolbar-aligned searchable root picker offers only the Root Ancestor and that person's Lineal descendants, shows the chosen root's total Outline people in a pill, and Reset Root returns directly to the Root Ancestor. Expand All and Condense All use two-line labels, while Condense Lineage collapses other branches but leaves the selected person's entire descendant subtree expanded. Tree and Outline switch between matching toolbars without a separate Outline heading, with icons above all control labels; Root/Expand/Condense/Condense Lineage use the shared SVG catalog and Expand/Condense remain directionally distinct after their 90-degree rotation. The Tree toolbar places the requested icon-over-label Reset action between In and Fit and restores a natural 100% view. The Tree and Outline print actions are both labelled Print and share the dedicated printer symbol, while the Tree view selector retains the tree symbol. Every print action first opens the in-app preview, whose header places an icon-and-text Print action beside Close; only that action invokes the native browser print dialog. Directory, Outline, and Groups render explicit Letter-portrait sheets with page counts and breaks in both preview and native print, matching the existing portrait Labels presentation; Tree alone uses landscape. Their preview DOM shares the native compact typography, grids, and card sizing so simulated page fill is accurate, while native portrait report sections rely on explicit breaks without a full-page height that can create alternating blank sheets. Outline prints only its currently expanded depth-first rows, omits the interactive chevron gutter, keeps Generation close to the person card, wraps long names instead of clipping them, and fits up to 44 ordinary compact rows per portrait sheet; Directory and Groups use content-weighted portrait pagination, with Directory fitting up to fourteen ordinary households per page. Groups owns the Root Ancestor/Generation 3 generation maps. Tree uses a 40px vertical generation gap, and its percentage steppers move in five-percent increments. Tree always requests letter landscape, always renders its print scale as 100% independently of the live zoom, and semantically paginates Full Tree with at most eight generation levels and ten people in any generation row per page; focused Lineage uses up to twelve levels or sixteen people across so ordinary selected branches remain on fewer pages. Boundary generations repeat between vertical sections; partner groups remain together when possible; descendant branches follow their parent into the same horizontal section; available parent/partner context repeats above the divided branch and is collision-packed without mutating the live Tree; page headers identify level/across sections. Detailed cards keep contact symbols, lifespan, and the Lineal symbol on one compact row, reserving a separate P-reference row only in Developer Mode. It otherwise preserves the current card, name, level, and line-visibility settings. Ancestors and Descendants default to 10; the obsolete persisted Descendants value of 3 migrates to 10 once. All four use dated titles; contact cards stay together within half-inch margins, and P ids/imported source/notes remain omitted.

## Version and lifecycle

Application versions are `major.minor.patch.build`. Every completed app change increments build. An explicit major/minor/patch release resets build to `1`; the first 1.0 cut is `1.0.0.1`, even if called “1.0.0” conversationally. Keep config identity/build, sole release entry, HTML queries/labels, service-worker cache/asset version, the checked-in Pages workflow name, and current docs identical. The workflow run title mirrors the versioned commit subject; its name exposes McFamily and the version in GitHub Mobile deployment notifications.
