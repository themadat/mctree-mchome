...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.19` uses schema v7, strict private-CSV onboarding, stable P-formatted McLineage records with direct lineage-parent references, explicit descendant date descriptors, absolute root-based generations, fuzzy search with persistent favorite people, a header-toggled sortable directory with A–Z jumps, direct numeric ancestor/descendant depths and zoom, optional display-only co-parent branches, a two-axis-scrollable lineage-ordered condensed/detailed Family Tree, a resizable desktop tree/profile split, compact Family line rows, birth-ordered profile family groups, a complete native CSV transfer format, a temporarily read-only family directory, and compact top-sibling-labelled print maps. There is no backend, account, GitHub Sync, cloud database, or runtime dependency.
...
- Lineage shows the two-digit source ID in person-to-root order with the selected person's first segment bold. Current cleaned sources use direct `lineage_parent_id` P references and retain `lineage_parent_name_full` only for readability; legacy name/prefix sources remain importable. A single Family line heading introduces paired name/reading rows whose cells share height and center vertically; each left cell reads `Name [## | G#]`, and the right cell carries only the relationship reading. Ancestor, sibling, and descendant totals span beneath both columns on one line. Every resolved name remains linked. George McMillen (1745) is G0, James is G1, George (1818) is G2, and Albon/Newton/Lucian are G3. Unknown positions omit the ordinal and read `Child of …`; the root reads `Root ancestor`.
