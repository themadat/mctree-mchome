...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.61` uses a latest-only schema v13 browser namespace and one strict dataset 16.0.x ZIP containing exact McPeople, McPlaces, McRelations, McResidences, and McMetadata CSV files. McMetadata carries an `editor`, `pii-viewer`, or `redacted-viewer` access mode; older dataset 16 packages without the row default to Editor once. Editor enables record mutation and audited cloud publishing, PII Viewer keeps the full record read-only, and Redacted Read-only physically removes places, residences, contacts, family Notes, and unstructured record notes. People use structured Birth/Current/Preferred/Maiden names, Lineal paths, and explicit NONE/UNKNOWN/UNKNOWN PRESUMED death descriptors. Parent relationships independently store Lineal/Non-Lineal role and biological/adopted/step/foster/guardian/unknown type; a child may have one Lineal and multiple Non-Lineal parents. Parent and partner links live only in McRelations; physical addresses live in McPlaces and reach people through McResidences. The existing fuzzy directory/search, favorites, coordinated directory/tree/profile workspace, name preferences, lineage-focused SVG tree, relationship styling, local recovery, themes, accessibility, installation/offline behavior, and print atlas remain. There is no custom backend, in-app account, cloud database, or runtime dependency. The title-bar Save & Share workspace shows current package access, replacement import, the three Editor-created handoff ZIPs, Editor-only private GitHub handoff, and simplified recent-change history.

The main product invariants are now the initialized import/download gate, directory/tree/profile workspace, single Notes modal, Settings Roadmap, local save/backup status, recovery copy, explicit private GitHub package handoff, themes, accessibility, install assets, and offline shell. Do not restore the removed Records interface, multi-note workspace, rich-text editor, or automatic browser-state synchronization.
...
- `assets/js/core/cloud.js`: explicit private GitHub package upload/download, patch versioning, audit continuity, and SHA conflict rejection.
...
- First launch initializes only from a validated dataset 16 McFamily ZIP containing exactly the five documented root CSV files and at least one person, either through local import or the explicit Download Latest action. There is no loose-CSV, demo-family, blank-family, JSON/GEDCOM, or bypass path.
- The gate and package access modes are not security or authentication. PII Viewer is a UI read-only guard around a plaintext full-data ZIP; only Redacted Read-only physically removes contact/Notes fields. GitHub collaborators/tokens control repository access and revocation; passwords and read/sign-in usage history remain a future-backend wishlist item.
- Maximum family size is 1,500 people.
- The ZIP is the one complete editable transfer artifact. Its five extracted CSVs are plaintext. Imports replace; they do not merge. Later replacement and person deletion create recovery first.
...
- Keep the runtime static, dependency-free, custom-backend-free, and hostable on ordinary GitHub Pages. The only family-data network path is the explicit private GitHub package transaction; never add background family-data sync.
- Save & Share in the title bar always shows the local package mode, replacement import, and recent in-package audit. Editor additionally sees three package exports and the private cloud handoff. The separate private repository stores only the canonical Editor ZIP. Download validates, saves recovery, opens locally, and saves the exact ZIP. Upload rejects viewer packages, validates the edited Editor ZIP, requires an actor/summary, preserves remote audit history, advances one `16.0.x` patch, appends `published-cloud-package`, revalidates, and uses the GitHub file SHA to reject stale writes. Tokens/settings remain outside packages and recovery; the service worker never caches cloud data.
...
- `features.familyEditing` is true, but mutations additionally require an Editor package. Keep Add, Connect, Edit, Delete, family-title, and Notes changes guarded in code and visibly disabled in PII Viewer and Redacted Read-only. Personal preferences and favorites remain local and available in all modes. Do not expose Set as home in the selected-person profile.
...
Follow `docs/TESTING.md`. At minimum cover strict ZIP onboarding, five-file round-trip/recovery and mismatch errors, people/place/residence/relationship links, cycle rejection, search, focus and overview graph, large synthetic layout, desktop/mobile overflow, keyboard/accessibility, print content, online reload, and offline shell. Stop preview servers afterward.
