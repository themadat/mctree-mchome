...
- Directory prints only the compact Directory of McMillen Clan contact cards. Outline is an interactive, depth-first descendant list with compact single-line rows for full Lineal names, life dates, the latest spouse or partner and relationship date, scan bars, branch collapse, root switching, and optional selected-lineage highlighting; people without partners have no placeholder copy. Its letter-landscape print includes the complete expanded outline. Groups owns the Root Ancestor/Generation 3 generation maps. Tree always requests letter landscape and paginates the live Full Tree or focused lineage at its current card, name, level, line-visibility, and zoom settings. Ancestors and Descendants default to 10; the obsolete persisted Descendants value of 3 migrates to 10 once. All four use dated titles; contact cards stay together within half-inch margins, and P ids/imported source/notes remain omitted.
