...
- Person search results show compact Preferred, Current, and Lineal name rows and expose an accessible, strongly highlighted Favorite toggle. The selected-person title bar has the same Favorite action. Favorite person IDs persist in normalized UI state and sort above other fuzzy matches. Favorites opens a one-time dropdown of every starred person without changing the search query, scope, placeholder, or header-button styling; typing immediately returns to ordinary family search. Developer Mode can save the starred P references to a small private `mcfamily-favorites` JSON file and restore that exact set independently of browser storage. Deleting a person removes their favorite ID.
