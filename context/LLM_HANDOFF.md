...
- `assets/js/core/state.js`: schema v12 normalization, fuzzy matching, places/residences, and validation; no historical state migrations.
...
- First launch accepts only a dataset 15 McFamily ZIP containing exactly the five documented root CSV files and at least one person. There is no loose-CSV, demo-family, blank-family, JSON/GEDCOM, or bypass path.
...
- Use only `mcfamily.state.v12` and `mcfamily.recovery.v4`; do not load, unwrap, or migrate earlier browser state or recovery snapshots. Startup removes only older versioned McFamily state/recovery keys so obsolete copies cannot exhaust the local-storage quota.
...
- Non-Lineal Lines is an off-by-default, persisted icon-and-two-line-label tree-toolbar button. Imported McRelations `non-lineal` parent rows normalize as internal `affinal`-kind parent relationships; the tree hides those edges while the button is off and draws them as lighter dashed branches while it is on. Its slash-drop symbol remains filled in both states. Lineal links stay solid in both states, and the toggle never mutates relationships. Ordinary UI labels use Lineal and Non-Lineal.
