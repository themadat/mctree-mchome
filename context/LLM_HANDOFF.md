...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.35` uses schema v8, strict current-schema-only private-CSV onboarding, stable P-formatted McLineage person rows with complete root-to-person lineage paths, explicit consanguineous and affinal parent references, JSON partner relationship records, death-record and age-based deceased inference, concise profile and print age details, absolute root-based generations, fuzzy search with persistent favorite people, a header-toggled directory with combinable status and kinship filters plus A–Z jumps, direct numeric ancestor/descendant depths up to 10 and zoom, toggled dashed affinal parent branches, a two-axis-scrollable lineage-ordered Family Tree with compact stacked-name cards, compact 20/50/30 default directory/tree/profile desktop splits with persistent resizing and developer position readouts, compact Family Line rows, relationship groups with generation and ordering context near the top of profiles, a complete native CSV transfer format, a temporarily read-only family directory, Title Case section headings, and compact top-sibling-labelled print maps. There is no backend, account, GitHub Sync, cloud database, or runtime dependency.
...
- Tree nodes use narrow cards that put every whitespace-separated name part on its own line and expand generation rows vertically as needed; condensed remains the default and detailed retains its extra context. Neither mode uses portrait or initial placeholders. Unknown lifespan years use `????`, and deceased nodes use a light-brown card. Generation rows sort by numeric source lineage ID before fallback placement. Recorded partners sit together when possible, with past partners chronologically on the left and exactly one current spouse on the right; only the current marriage is solid; never-married partnerships are dashed, partnerships that ended for an unrecorded reason are drawn as a `????` glyph line with no stroke, and divorced, separated, and widowed links are dotted. Current cleaned McLineage imports derive those states from each JSON relationship type and end reason; any recorded end reason, including `UNKNOWN`, produces a past state so the person keeps no current partner. Focus view has independent numeric Ancestors and Descendants depth controls with their labels above the number fields, grouped Out/In/Fit buttons, and an editable zoom percentage; person search selections return to Focus.
- Hide 99 Lineage is an on-by-default, persisted tree-toolbar checkbox. It removes people whose source `lineage_id` is `99`, plus anyone whose every relationship points only at them, from both tree modes; the focused person is never hidden. Those people stay in the directory, search, print, and counts.
...
- A source `relationship_type` of `partnership` is a never-married partnership. It reads Never married in profiles and print and draws the dashed partner line whatever its end reason, so the `????` treatment stays with ended marriages.
- Affinal Lines is an off-by-default, persisted tree-toolbar checkbox. Imported `parent_affinal_person_id` links are stored as `affinal`-kind parent relationships; the tree hides those edges while the checkbox is off and draws them as lighter dashed branches while it is on. Consanguinity links stay solid in both states, and the toggle never mutates relationships. Normalization re-derives each cleaned-source parent kind from the relationship's own `parent_consanguinity_person_id`/`parent_affinal_person_id` field, so already-saved workspaces classify correctly without a re-import. Profiles list the recorded consanguinity and affinity parents only; there is no partnership-based co-parent guess.
...
- Profiles and print carry a Marital Status row derived from the most recent partnership: married, widowed, divorced, separated, never married for a partnership that was never a marriage, and unknown for an ended partnership with no recorded reason or for a person with no partnership records. The vocabulary and the partner-status mapping live in `config.maritalStatuses` and `config.maritalStatusByPartnerStatus`.
