...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.58` uses a latest-only schema v13 browser namespace and one strict dataset 16 ZIP containing exact McPeople, McPlaces, McRelations, McResidences, and McMetadata CSV files. People use structured Birth/Current/Preferred/Maiden names, Lineal paths, and explicit NONE/UNKNOWN/UNKNOWN PRESUMED death descriptors. Parent relationships independently store Lineal/Non-Lineal role and biological/adopted/step/foster/guardian/unknown type; a child may have one Lineal and multiple Non-Lineal parents. Parent and partner links live only in McRelations; physical addresses live in McPlaces and reach people through McResidences. McMetadata declares package/schema versions and counts and preserves import/export audit history. The existing fuzzy directory/search, favorites, coordinated directory/tree/profile workspace, name preferences, lineage-focused SVG tree, relationship styling, local recovery, Notes, themes, accessibility, install/offline behavior, and print atlas remain. There is no backend, account, GitHub Sync, cloud database, or runtime dependency.
...
- Preserve the schema-v12 privacy, validation, latest-only storage, accessibility, and offline invariants below.
...
- First launch accepts only a dataset 16 McFamily ZIP containing exactly the five documented root CSV files and at least one person. There is no loose-CSV, demo-family, blank-family, JSON/GEDCOM, or bypass path.
