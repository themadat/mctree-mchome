...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.51` uses a latest-only schema v10 browser namespace and exact 36-column McLineage v12 onboarding, structured Birth/Current/Preferred/Maiden names, Preferred → Current → Birth display fallback, compact four-row profile names, Preferred-or-Legal-or-Lineal and Short-or-Full tree-name controls, stable P-formatted McLineage person rows with complete root-to-person lineage paths, user-facing Lineal and Non-Lineal relationships, JSON partner relationship records, death-record/age/early-generation/partner-based presumed-deceased inference, perspective-aware older-marriage status, compact parallel prior-partner placement, aligned compact profile-property values with natural-language ages, absolute root-based generations, fuzzy search with persistent favorite people selectable from search or profiles and a one-shot favorites dropdown, a header-toggled directory with combinable status and kinship filters plus A–Z jumps, selection-aware icon-labelled Full Tree/Lineage and Details/Summary controls, grouped symbol-labelled ancestor/descendant depths defaulting to 10, toggled dashed Non-Lineal parent branches with fixed symbols, standard-fill Lineal cards with bold muted-red outlines and lifespan-row marks, faded-red Lineal parent edges, gold partner edges with question-mark unknown status, a two-axis-scrollable lineage-ordered Family Tree, compact 20/50/30 default directory/tree/profile desktop splits with persistent resizing and drag-only developer position readouts, compact Family Line rows, relationship groups with generation and ordering context near the top of profiles, a complete native CSV v2 transfer format, a temporarily read-only family directory, Title Case section headings, and six-column, Generation-3-line-grouped print maps with deceased shading and a Developer Mode preview. There is no backend, account, GitHub Sync, cloud database, or runtime dependency.
...
- Preserve the schema-v10 privacy, validation, latest-only storage, accessibility, and offline invariants below.
...
- `assets/js/core/state.js`: schema v10 normalization, fuzzy matching, and validation; no historical state migrations.
...
- First launch accepts only the exact 36-column McLineage v12 CSV or a current native `mcfamily-csv-v2` CSV containing at least one person. There is no demo family, blank-family path, JSON/GEDCOM import, or bypass.
...
- Use only `mcfamily.state.v10` and `mcfamily.recovery.v2`; do not load, unwrap, or migrate earlier browser state or recovery snapshots.
