...
This repository is McFamily, a static, local-first family atlas. Version `0.0.1.60` uses a latest-only schema v13 browser namespace and one strict dataset 16.0.x ZIP containing exact McPeople, McPlaces, McRelations, McResidences, and McMetadata CSV files. People use structured Birth/Current/Preferred/Maiden names, Lineal paths, and explicit NONE/UNKNOWN/UNKNOWN PRESUMED death descriptors. Parent relationships independently store Lineal/Non-Lineal role and biological/adopted/step/foster/guardian/unknown type; a child may have one Lineal and multiple Non-Lineal parents. Parent and partner links live only in McRelations; physical addresses live in McPlaces and reach people through McResidences. McMetadata declares package/schema versions and counts and preserves audit history. The existing fuzzy directory/search, favorites, coordinated directory/tree/profile workspace, name preferences, lineage-focused SVG tree, relationship styling, local recovery, Notes, themes, accessibility, installation/offline behavior, and print atlas remain. There is no custom backend, in-app account, cloud database, or runtime dependency. The title-bar Save & Share workspace combines local backup/import, visible private GitHub connection and dataset status, validated cloud handoff, and a simplified recent-change history in one scrollable dialog.
...
- The gate is not security or authentication. GitHub collaborators/tokens control repository access and revocation; application owner/editor/viewer roles and read/sign-in usage history remain a future-backend wishlist item.
...
- Audit in the title bar opens Cloud Records & Audit. A separate private repository stores one latest ZIP. Download validates, saves recovery, opens locally, and saves the exact ZIP. Upload validates the edited ZIP, requires an actor/summary, preserves remote audit history, advances one `16.0.x` patch, appends `published-cloud-package`, revalidates, and uses the GitHub file SHA to reject stale writes. Tokens/settings remain outside packages and recovery; the service worker never caches cloud data.
