(function () {
  ...
  const CONFIG = {
    ...
    help: [
      { id: "start", title: "Getting started", section: "Basics", keywords: "start passphrase password access owner editor viewer encrypted first launch", html: "<p>Open the ordinary McFamily link, choose the name assigned by the Owner, and enter your private passphrase. The app downloads ciphertext, decrypts it only in this browser, and validates the complete five-file family record before opening. Recipients do not need a ZIP. Before the first encrypted vault exists, the Owner uses one private recovery ZIP to initialize their browser and publish hosted access.</p>" },
      ...
      { id: "access-packages", title: "Passphrases and access", section: "Data", keywords: "password passphrase access owner editor username pii private viewer redacted revoke rotate", html: "<p>Everyone uses the same public McFamily link. <strong>Owner</strong> can edit, publish, and manage access. Each named <strong>Editor</strong> has a separate passphrase, can edit and publish, and is automatically named in publication history. Separately named <strong>Private Viewers</strong> see full profile details read-only, while named <strong>Redacted Viewers</strong> decrypt only a copy with places, contacts, and unstructured private notes physically removed. Neither viewer role can open family Notes. Owners can add, rename, rotate, or revoke every recipient independently. Revocation blocks the next sign-in after reload but cannot erase information already seen or copied.</p>" },
