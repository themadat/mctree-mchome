(function () {
  ...
  const CONFIG = {
    ...
    help: [
      ...
      { id: "print", title: "Print or save a PDF", section: "Family", keywords: "print pdf atlas directory household address contact lineage family maps generation developer preview", html: "<p>Choose <strong>Print / Save PDF</strong> to build the atlas and open the browser print dialog. After the cover, Person Directory includes people with at least one phone, email, or address, groups them by primary address, and sorts households by the main person's Display Last Name. Main people and partners occupy separate rows with their own phone and email; other same-address residents follow as a muted name list. Display Names are used throughout, only Lineal names are bold, and deceased partners retain the concise italic death marker. Six-column Family Maps still include people omitted from the contact directory. In Developer Mode, the same action opens an in-app preview instead of printing.</p>" },
