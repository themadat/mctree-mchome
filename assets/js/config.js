(function () {
  ...
  const CONFIG = {
    ...
    controls: {
      ...
      maxPrintTreeLevels: 8,
      maxPrintTreePeopleAcross: 10,
    ...
    releases: [
      {
        ...
        title: "Known birth order with unknown birthdays",
        summary: "Record an unknown or incomplete birthday's sibling position while keeping Lineage IDs calculated and unique.",
        features: ["Birth order placement in the person editor", "Unknown Lineal Births review in Data Cleanup"],
        improvements: ["Shared sibling ordering across profiles, Tree, Outline, and print", "Birth positions preserved in all access-mode packages"],
        fixes: ["Reject duplicate Lineage IDs on import", "Recalculate sibling and descendant lineage after birthday or birth-order edits"],
        knownIssues: []
      }
    ],
    ...
    help: [
      ...
      { id: "tree", title: "Exploring the Family Tree", section: "Family", keywords: "tree focus overview zoom reset pan scroll generation ancestor descendant condensed detailed co-parent non-lineal resize preferred legal birth name", html: "<p>Focus shows the selected person and nearby generations; Full Tree shows every component and closes the profile. Each descendant branch centers beneath its recorded parent or parent couple, retains the parents' left-to-right order, and moves only as needed to avoid neighboring family branches. Ancestor and Descendant levels default to 10; this release replaces the obsolete saved Descendants value of 3 once, while a level you deliberately choose afterward remains saved. Summary cards show names; Details adds lifespan and Lineal/contact symbols. Name Preferences chooses Preferred, Legal, or Lineal names and Short or Full length. Gold links show partner status, muted-red links show Lineal parents, and Non-Lineal Lines reveals other recorded parents. Use the Key, scroll, pan, zoom, Reset for 100%, Fit, and the desktop dividers to explore or resize the tree.</p>" },
      { id: "people", title: "People and relationships", section: "Family", keywords: "people list favorites star search sort filter blood lineal non-lineal alphabet address autofill phone email parent child partner ancestry lineage generation names preferred legal birth maiden", html: "<p>Use <strong>List</strong> after Add to open the searchable people pane. Filter by living status, Lineal scope, address, phone, or email; sort by first or last name; and use the A–Z rail to jump. Global search shows Preferred, Current, and Birth names. Star people from search or a profile, then choose the Favorites star inside the right side of search to list them. Admin can restore a Favorites file in Developer Mode. Profiles contain Names, auto-generated Lineage, Relationships, contacts, and person-level editor notes. The Relationships section uses Add, Edit, and Delete: Add separates new people from existing people, while Edit and Delete group choices under Parents, Partners, and Children. A new person created from a relationship starts with <strong>Autofill from related person</strong> checked and the related person's current address copied into the form; uncheck it to start without that address. Imported Source appears only to editors in Developer Mode. Lineal parents generate the child’s Lineage ID automatically. For an unknown or incomplete birthday, edit Birth order among Lineal siblings to place the person before or after a sibling. Full birthdays restore date order; changing the Lineal parent clears the placement. Unrecorded order is provisional.</p>" },
      { id: "cleanup", title: "Reviewing Data Cleanup", section: "Data", keywords: "cleanup invalid placeholder unknown incomplete relationship partner address birthday birth death name lineage editor quality", html: "<p>Editors can open <strong>Settings → Data Cleanup</strong> to review suspicious or incomplete records. Expand any category to inspect it, then open the affected person or relationship. Placeholder-name issues identify every affected Birth, Current, Preferred, or Maiden name part even when another valid name exists; intentionally blank Unknown people remain supported. Unknown dates have no known digits; incomplete dates contain some date information but are not a full YYYY-MM-DD value. Partner-address mismatches include living people in an ongoing partnership only when both have current postal addresses and those addresses differ. These checks do not rewrite family data automatically.</p>" },
      { id: "print", title: "Print Directory, Outline, Groups, or Tree", section: "Family", keywords: "print pdf directory outline descendants spouse household address phone contact lineage family groups maps generation tree zoom developer preview portrait landscape", html: "<p><strong>Directory</strong> prints only the compact household contact cards in <strong>Directory of McMillen Clan</strong>. <strong>Outline</strong> opens an indented descendant view with stable Root-Ancestor-relative generation numbers, compact stacked life dates, the latest spouse beside each person, shared Name Preferences, a compact searchable Lineage-only root picker with a people-count pill, Reset Root, branch controls, Condense Lineage, and optional selected-lineage highlighting. Condense Lineage keeps the selected person's entire descendant branch expanded while condensing unrelated branches; its Print action includes only the currently expanded rows for the chosen root and fits up to 44 ordinary compact rows on each portrait page. <strong>Groups</strong> prints the six-column generation maps organized by Root Ancestor and Generation 3 Line. Directory, Outline, Groups, and Labels use Letter portrait pages; Tree alone uses Letter landscape. Their previews use the same compact styles and page density as native print. Full Tree uses no more than eight generation levels or ten people across per page; the selected or focused Lineage can use up to twelve levels or sixteen people across. Descendant branches stay with their parent when the tree divides horizontally, with the necessary ancestor context repeated. Tree output follows the active Card View, name preferences, levels, and Non-Lineal and ?? Lineal visibility, but always prints at 100% regardless of the interactive zoom. Every print action opens an in-app preview first; choose Print beside Close to open the browser print dialog. Each output uses a dated title.</p>" },
      ...
      { id: "cloud", title: "Saving the encrypted family", section: "Data", keywords: "cloud github save audit encrypted vault publish latest patch editor username token conflict json path changes update", html: "<p><strong>Save</strong> lets Admin and named Editor access publish the current family to the ciphertext-only public <code>mcdata</code> repository. Signed-in identity, dataset date, and GitHub status remain visible in the top bar; choose GitHub there to expand connection settings. The publishing row shows publisher, next patch, Bulk Upload, and Update in working order. McFamily compares the local family with the record opened from the vault, places every unpublished family change on its own line, and enables Update only when changes exist. Admin publications are recorded as Admin; Editor publications use the signed-in Editor name. McFamily generates one <code>data/mcfamily/McFamily-access.json</code> file containing the access grants plus separately encrypted full and Viewer family packages. The fine-grained GitHub token stays outside the vault.</p>" },
