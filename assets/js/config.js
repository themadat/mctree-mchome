(function () {
  ...
  const CONFIG = {
    ...
    releases: [
      {
        ...
        title: "Compact search and Save workflow",
        summary: "Favorites now lives inside search, List follows Add, partner details read more naturally, and Save presents status and publishing controls in a compact working order.",
        features: ["Moved signed-in identity, dataset date, and GitHub connection status into the Save dialog header"],
        improvements: ["Anchored Favorites after the search shortcut, ordered the toolbar as Save, Add, List, Directory, and placed publisher, next patch, Bulk Upload, and Update on one line"],
        fixes: ["Placed partner edit controls before status context, changed partner context to status before year, and made every calculated publication change use a full-width row"],
        knownIssues: []
      }
    ],
    ...
    help: [
      ...
      { id: "people", title: "People and relationships", section: "Family", keywords: "people list favorites star search sort filter blood lineal non-lineal alphabet address phone email parent child partner ancestry lineage generation names preferred legal birth maiden", html: "<p>Use <strong>List</strong> after Add to open the searchable people pane. Filter by living status, Lineal scope, address, phone, or email; sort by first or last name; and use the A–Z rail to jump. Global search shows Preferred, Current, and Birth names. Star people from search or a profile, then choose the Favorites star inside the right side of search to list them. Admin can restore a Favorites file in Developer Mode. Profiles contain Names, auto-generated Lineage, Relationships, contacts, and person-level editor notes. Partner rows place Edit immediately before <code>(status :: year)</code>. Imported Source appears only to editors in Developer Mode. Editors can add or connect parents, partners, and children; Lineal parents generate the child’s Lineage ID automatically.</p>" },
      { id: "print", title: "Print Directory, Groups, or Tree", section: "Family", keywords: "print pdf directory household address phone contact lineage family groups maps generation tree zoom developer preview", html: "<p><strong>Directory</strong> prints only the compact household contact cards in <strong>Directory of McMillen Clan</strong>. <strong>Groups</strong> prints the six-column generation maps organized by Root Ancestor and Generation 3 Line. <strong>Tree</strong>, at the far right of the Family Tree controls, paginates the currently displayed tree: Full Tree includes the complete visible family, while Lineage uses the selected or focused person. Tree output follows the active Card View, name preferences, levels, Non-Lineal and ?? Lineal visibility, and Zoom; zoom out if a large Full Tree would require too many pages. Each output opens the browser print dialog with a dated <code>McFamily-Directory</code>, <code>McFamily-Groups</code>, or <code>McFamily-Tree</code> title. In Developer Mode, the same actions open an in-app page preview instead.</p>" },
