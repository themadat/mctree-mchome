(function () {
  ...
  const CONFIG = {
    ...
    help: [
      ...
      { id: "tree", title: "Exploring the Family Tree", section: "Family", keywords: "tree focus overview zoom pan scroll horizontal vertical generation ancestor descendant home person condensed detailed co-parent non-lineal resize", html: "<p>Focus view shows the selected person and nearby generations; choosing a person from search returns here automatically. Select any person to recenter the Family Tree, set the grouped Ancestor and Descendant depth numbers, or switch between condensed and detailed cards. Both generation depths default to 10. Narrow cards place each name word on its own line; Lineal people keep the standard living or deceased card fill and use a bold muted-red outline plus a small lineage symbol beside the lifespan. Selection temporarily replaces that outline with the same accent border used for every person. Faded muted-red parent edges follow the Lineal bloodline. <strong>Non-Lineal Lines</strong> adds the lighter dashed branch from each recorded Non-Lineal parent to their child; its icon fills while the lines are shown. Gold partner links distinguish current marriages (solid), previous marriages (dashed), never-married relationships (dotted), and unknown relationships (question marks). Up to two prior partners appear at two-thirds size on the left: one is centered; two align with the top and bottom of the full-size spouse and use evenly spaced parallel links. The current or latest death-ended spouse remains full-size on the right. The Key at the lower right of the module names each line and can be collapsed. <strong>Show ?? Lineal</strong> reveals people whose stored source lineage is 99, plus anyone connected only to them; the value appears as ?? in the interface and its icon fills while those people are shown. They always remain in the directory and search. On desktop, drag the divider beside the selected person panel to resize both modules; Developer Mode shows its percentage only while dragging. Scroll horizontally or vertically to explore larger layouts; enter a zoom percentage or use the right-aligned Out, In, and Fit actions.</p>" },
      { id: "people", title: "People and relationships", section: "Family", keywords: "people directory favorites star search sort filter blood lineal non-lineal alphabet address phone email parent child partner ancestry lineage generation", html: "<p>Use Directory to the left of Search to open or close the pane. Its title-bar search contains the current result count. Filter By combines any living-status, Lineal, and Non-Lineal checkboxes; Sort By switches between first and last name; the A–Z rail follows the filtered results. Select the star beside a person search result to pin them above other matches. Favorites to the right of Search opens a one-time dropdown of every starred person without changing the search scope. The person panel closes and deselects with its X; selecting a person in the Family Tree reopens it. Lineage uses a compact Family Line with each name followed by its lineage number and generation; readings use the generation and the parent's first name, such as Gen 6, 5th Child of Max. Lineage sits directly under identity details, above Relationships. Identity details show every property, use UNKNOWN for missing values, and keep Age compact on one line. An otherwise unknown-status partner of any deceased person is inferred as presumed deceased, as is an unknown Lineal member in Generation 0 through 4. A deceased spouse in a marriage ended by death reads Married while the surviving spouse reads Widowed. Relationship groups label parent, sibling, and child generations; each Partners row adds its year and perspective-aware status; parents identify recorded Lineal or Non-Lineal roles, siblings and children show birth order and year, and partners show marriage year. Partners puts the displayed spouse first in bold before reverse-ordered prior partners. Notes follow Relationships, and Imported Source finishes the profile. Generations are rooted at George McMillen (1745) as G0.</p>" },
      { id: "print", title: "Print or save a PDF", section: "Family", keywords: "print pdf atlas directory lineage family maps generation developer preview", html: "<p>Choose <strong>Print / Save PDF</strong> to build the atlas and open the browser print dialog. Its compact opening flows into six-column Family Maps; George McMillen (1745) is Generation 0, and Generation 4 and later are grouped beneath Generation 3 family lines. Lineal cards have a faded-red outline; stronger Newton, Albon, and Lucian orientation highlights apply only to Lineal people, deceased entries use brown shading, and unresolved 99/?? lineage branches are omitted. Jon Couts remains in the directory but is not presented as a map root. The dense three-column Person Directory shows only each full name, styled Lineage ID, and root-to-person first-name progression. In Developer Mode, the same action opens an in-app preview instead of printing.</p>" },
